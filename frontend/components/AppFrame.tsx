...
export function AppFrame({ children }: { children: React.ReactNode }) {
  ...
  const home = user?.role === "ADMIN" ? "/admin/dashboard" : user?.role === "MENTOR" ? "/mentor/dashboard" : "/student/dashboard";
  return <div className="min-h-screen bg-slate-50">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
      ...
      <div className="flex items-center gap-3">{user?.role === "ADMIN" && <><Link href="/admin/student-accounts" className="text-sm font-semibold text-blue-700 hover:text-blue-900">Student accounts</Link><Link href="/admin/mentor-accounts" className="text-sm font-semibold text-blue-700 hover:text-blue-900">Mentor accounts</Link></>}<button onClick={signOut} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold hover:bg-slate-50">Sign out</button></div>
    </div></header>
    {children}
  </div>;
}
